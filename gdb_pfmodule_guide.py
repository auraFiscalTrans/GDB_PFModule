...
availability_ranking = pfmodule_dataframe[(pfmodule_dataframe["hierarchy_level"] == 1) \
                        & (pfmodule_dataframe["indicator_name"] == "Availability (PF): Budget and spend data")] 

availability_ranking_sorted = availability_ranking.sort_values("score", ascending=False)
availability_ranking_df = availability_ranking_sorted.dropna(subset=["score"])

###Task 3: Governance Indicator World Rank 
governance_ranking = pfmodule_dataframe[(pfmodule_dataframe["hierarchy_level"] == 1) \
                        & (pfmodule_dataframe["indicator_name"] == "Governance (PF): Public finance data")] 
governance_ranking_sorted = governance_ranking.sort_values("score", ascending=False)
governance_ranking_df = governance_ranking_sorted.dropna(subset=["score"])
